...
const createAllFields = () => {
    ...
    if(defaults.form.formId) {
        fieldNodeList = document.querySelectorAll(`#${defaults.form.formId} [data-val-rules]`);
    } else {
    ...
    for(let field of fieldsToAdd) {
        ...
        if(field.type === 'checkbox' || field.type === 'radio') {
            let collectAll = document.querySelectorAll('input[name^="' + field.name.replace(/[^a-z0-9 ,.?!]/ig, '') +'"]')
        ...
        if(formFields.findIndex(fieldToCheck => fieldToCheck.name === field.name) === -1) {
...
const fieldChanged = (event) => {
    let index = defaults.formFields.findIndex((field) => { return field.name === event.target.name})

    if( event.target.type === 'checkbox' || event.target.type === 'radio') {
        const validCheckboxes = document.querySelectorAll('input[name="' + event.target.name +'"]:checked')
