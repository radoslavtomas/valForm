...
let defaults = {
    ...
    messages: {
        ...
        max_years_in_past:
            "The %s date must be no more than %s years in the past.",
    ...
    for (let field of fieldsToAdd) {
        ...
        if (
            formFields.findIndex(
                fieldToCheck => fieldToCheck.name === field.name
            ) === -1
        ) {
...
const addField = field => {
    let obj = {
        name: field.name,
        display: field.dataset.valDisplay
            ? field.dataset.valDisplay
            : field.name,
...
const getValidationErrorMessage = (rule, fieldName, param = null) => {
    ...
    if (param) {
        if (param.split(":").length > 1) {
            ...
            console.log(param);
            if (
                Object.keys(defaults.formFieldsNames).indexOf(
                    param.replace("=", "")
                ) >= 0
            ) {
                errorMessage = errorMessage.replace(
...
const dispatchFieldValidationEvent = (element, fieldName) => {
    let fieldData = defaults.formFields.filter(
...
let hooks = {
    ...
    date_greater_than: (field, param) => {
        ...
        if (message.indexOf(add) !== -1) {
            defaults.messages.date_greater_than = message.replace(
                " " + add,
                ""
            );
        }
    ...
    years_between: (field, param) => {
        ...
        console.log(
            calculateDiffInYears(fieldDate, fieldAgainstDate) >=
                parseFloat(years)
        );
